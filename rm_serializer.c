...
extern char * serializeRelationInfo(relationInfo *rInfo)
{
	VarString *result;
	MAKE_VARSTRING(result);
	APPEND(result, "FirstRecord <%i> LastRecord <%i> LengthOfRecord <%i> RelationName <%s> RowCount <%i> RowsPerPage <%i> SchemaLength <%i>",
		rInfo->firstRecord, rInfo->last, rInfo->lengthofrecord, rInfo->relationName, rInfo->rowCount, rInfo->slotperpage, rInfo->schemaLength
		);

	char *resultString;				\
		GET_STRING(resultString, result);
	return resultString;
}
